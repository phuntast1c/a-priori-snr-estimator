...
def compress(
    snr_dB: torch.Tensor, mu: torch.Tensor, sigma: torch.Tensor
) -> torch.Tensor:
    """
    Compute the compressed a priori SNR (Equation 11, Zhang, Q., Nicolson, A. M., Wang, M., Paliwal, K. & Wang, C.-X. DeepMMSE: A Deep Learning Approach to MMSE-based Noise Power Spectral Density Estimation. IEEE/ACM Trans. Audio, Speech and Lang. Proc. 28, 1404–1415 (2020).)

    Args:
    xi_dB (torch.Tensor): A priori SNR in dB tensor.
    mu_k (torch.Tensor): Mean parameter tensor.
    sigma_k (torch.Tensor): Standard deviation parameter tensor.

    Returns:
    torch.Tensor: Mapped a priori SNR.
    """
    term = (snr_dB - mu[None, :, None]) / (sigma[None, :, None] * SQRT_2)
    return 0.5 * (1 + term.erf())


def expand(
    compressed_snr: torch.Tensor, mu: torch.Tensor, sigma: torch.Tensor
) -> torch.Tensor:
    """
    Compute the expanded a priori SNR estimate (NOT in dB anymore!!!) (Equation 12, Zhang, Q., Nicolson, A. M., Wang, M., Paliwal, K. & Wang, C.-X. DeepMMSE: A Deep Learning Approach to MMSE-based Noise Power Spectral Density Estimation. IEEE/ACM Trans. Audio, Speech and Lang. Proc. 28, 1404–1415 (2020).)

    Args:
    mapped_snr (torch.Tensor): Mapped a priori SNR tensor.
    mu_k (torch.Tensor): Mean parameter tensor.
    sigma_k (torch.Tensor): Standard deviation parameter tensor.

    Returns:
    torch.Tensor: A priori SNR estimate.
    """
    erf_inv_term = (2 * compressed_snr - 1).clamp(-1, 1).erfinv()
    return 10 ** (
        (sigma[None, :, None] * SQRT_2 * erf_inv_term + mu[None, :, None]) / 10
    )
